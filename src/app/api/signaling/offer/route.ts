...
export async function POST(request: NextRequest) {
  ...
  if (!body?.offer?.sdp || body.offer.type !== "offer" || !body.token) {
    return Response.json(
      { error: "Expected an SDP offer and room token." },
      { status: 400 },
    );
  }
  ...
  try {
    upstream = await fetch(`${MEDIA_URL}/v1/webrtc/publish`, {
      method: "POST",
      headers: {
        authorization: `Bearer ${body.token}`,
        "content-type": "application/json",
      },
      body: JSON.stringify({ offer: body.offer, roomId }),
    });
  } catch (error) {
    return Response.json(
      {
